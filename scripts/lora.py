...
"""
LoRA (Low-Rank Adaptation) training for Titans MLX models on Apple Silicon.
...
- Full training loop with chat data pipeline, checkpointing, and CLI

Usage:
    # LoRA fine-tune with a chat dataset
    uv run python scripts/lora.py --dataset allenai/Dolci-Instruct-SFT \
        --model mac --tokenizer gpt2 --dim 256 --num-layers 4

    # LoRA with custom targets and rank
    uv run python scripts/lora.py --dataset allenai/Dolci-Instruct-SFT \
        --lora-targets attn,ffn --lora-rank 16 --lora-alpha 32
...
def tokenize_chat(
    ...
    Returns:
        Dict with "input_ids", "labels", "loss_mask".
    """
...
class LoRAConfig:
    ...
    # Training -- LoRA defaults differ from SFT
    epochs: int = 1
    max_steps: int = -1
    batch_size: int = 4
    ...
    log_every: int = 10
    wandb: bool = False
    wandb_project: str = "titans-mlx-lora"
...
def train(
    ...
    """Main LoRA training loop (streaming only)."""
    ...
    pbar = tqdm(total=total_steps, initial=global_step, desc="LoRA Training")
    pbar.refresh()
    ...
    # Save final adapters
    adapter_dir = checkpoint_dir / "final_adapters"
    ...
    save_adapters(model, adapter_dir / "adapters", adapter_meta)
    logger.info(f"Saved final adapters to {adapter_dir}")
    ...
    if config.merge_and_save:
        ...
        logger.info(f"Merged LoRA weights and saved to {merge_path}")

    # Final checkpoint (full model with LoRA layers)
    save_checkpoint(
        model,
        optimizer,
        config,
    ...
    logger.info(f"LoRA training completed in {elapsed / 3600:.2f} hours")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="LoRA Fine-Tuning for Titans MLX models",
    ...
    parser.add_argument("--wandb-project", type=str, default="titans-mlx-lora")
    ...
    logger.info(
        ...
        f"{trainable_params / max(total_params, 1) * 100:.2f}% of total)"
    )
