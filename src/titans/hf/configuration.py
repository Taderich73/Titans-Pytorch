...
class TitansMACConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        titans_schema_version: int = TITANS_SCHEMA_VERSION,
        **kwargs,
    ) -> None:
        ...
        # attribute so downstream consumers can branch on it. Kwarg
        # default is ``TITANS_SCHEMA_VERSION`` so loading an older config
        # JSON that lacks the field picks up the current version (which
        # matches today's layout exactly).
