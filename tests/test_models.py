...
class TestChunkCheckpointing:
    ...
    def test_titans_mag_multi_chunk_checkpointed_parity(self, device):
        """TitansMAG checkpointed forward must match non-checkpointed."""
        torch.manual_seed(0)
        config_ref = TitansConfig(
            dim=64,
            num_heads=4,
        ...
        torch.manual_seed(0)
        ref_model = TitansMAG(config_ref).to(device).eval()
        torch.manual_seed(0)
        ck_model = TitansMAG(config_ck).to(device).eval()
        ck_model.load_state_dict(ref_model.state_dict())
